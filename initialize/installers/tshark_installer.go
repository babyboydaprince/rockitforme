package installers

import (
	"fmt"
	"os"
	"os/exec"
	_ "rockitforme/utils"
)

const tshark = "tshark"

// TODO - EXAMPLE OF DEPENDENCY SET UP
var tsharkDependencies = map[string][]string{
	"debian": {"sudo", "ruby"},
	"fedora": {"sudo", "ruby"},
	"arch":   {"sudo", "ruby"},
}

func TsharkInstall(check string, OpSystem string) bool {
	switch check {
	case "dependencies":
		deps, ok := tsharkDependencies[OpSystem]
		if !ok {
			return false
		}
		if err := checkTsharkDependencies(deps); err != nil {
			fmt.Printf("Error: %v\n", err)
			os.Exit(1)
		}
	case "installed":
		if isTsharkInstalled() {
			return true
		} else {
			if err := installTshark(OpSystem); err != nil {
				fmt.Printf("Error: %v\n", err)
				os.Exit(1)
			}
			return false
		}
	default:
		fmt.Println("Invalid check argument. Please use 'dependencies' or 'installed'.")
		os.Exit(1)
	}
	return false
}

func isTsharkInstalled() bool {
	_, err := exec.LookPath(tshark)
	return err == nil
}

func installTshark(OpSystem string) error {
	switch OpSystem {
	case "debian":
		cmd := exec.Command("sudo", "apt", "install", "wireless-tools", "-y")
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		return cmd.Run()
	//case "fedora":
	//	cmd := exec.Command("sudo", "dnf", "install", "wireless-tools", "-y")
	//	cmd.Stdout = os.Stdout
	//	cmd.Stderr = os.Stderr
	//	return cmd.Run()
	case "arch":
		cmd := exec.Command("sudo", "pacman", "-S", "--noconfirm", "wireless_tools")
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		return cmd.Run()
	default:
		return fmt.Errorf("unsupported Linux distribution: %s", OpSystem)
	}
}

func checkTsharkDependencies(dependencies []string) error {
	for _, dep := range dependencies {
		_, err := exec.LookPath(dep)
		if err != nil {
			return fmt.Errorf("dependency not found: %s", dep)
		}
	}
	return nil
}
